...
if [ -f "memory/constitution.md" ]; then
  ...
  echo "[!] No constitution found. Run 'flowspec init --here' to create one."
fi
```
...
**Message**:
```
[!] Constitution has N unvalidated sections:
  - Section 1 name
  - Section 2 name
  - Section 3 name
...
**Message**:
```
[!] Constitution Validation Recommended
...
1. Log warning: `[!] Skipping constitution validation (--skip-validation)`
...
[!] Constitution has 3 unvalidated sections:
...
[!] Skipping constitution validation (--skip-validation)
...
1. [X] Skipping validation check entirely
2. [X] Treating all tiers the same
3. [X] Generic error messages without context
4. [X] No option to bypass (sometimes needed)
5. [X] Checking constitution in middle of workflow

Instead:
1. [Y] Always check at command start
2. [Y] Apply tier-specific enforcement
3. [Y] Show exact sections needing validation
4. [Y] Support `--skip-validation` for emergencies
5. [Y] Check early, fail fast

## Troubleshooting

### Constitution Not Found
```
[!] No constitution found at memory/constitution.md
...
```
[i] No TIER comment found in constitution - defaulting to Medium tier.
...
```
[!] Found NEEDS_VALIDATION markers but couldn't extract section names.
